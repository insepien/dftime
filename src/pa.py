...
def p_af_sigma_lmbh(a,logmbh,t,a0,R=1):
    """calculate int p(af, sigma|mbh) d~sigma
        (same as above but in terms of logmbh for mbh range intergration)"""
    mbh = 10**logmbh
    prefactor = 1/(np.sqrt(2*np.pi)*epsilon/beta) 
    sigma_star = mbh*t/(gamma/R)/(a0**2-a**2)/200
    exp_term = np.exp(-(np.log10(sigma_star) - 1/beta*logmbh + alpha/beta)**2 / (2*epsilon**2/beta**2))
...
def p_af_sigma_a0_mbhrange_unif_fast(amin,amax,mbhmin,mbhmax,t,R,af_grid=None):
    """for a0~U(amin,amax), mbh~U(mbhmin,mbhmax)
        marginalize over sigma and a0 to find p(af|mbh) for 1 rate """
    if af_grid is None:
        af_grid = np.linspace(0,amax+3,200)
    a0_grid = np.linspace(0,af_grid.max()+3,2000)
    logmbh_grid = np.linspace(mbhmin,mbhmax,50)
    AF,A0,LMBH = np.meshgrid(af_grid,a0_grid,logmbh_grid,indexing="ij")

    pa_sigma_grid = p_af_sigma_lmbh(AF, LMBH, t, A0, R)
    print(pa_sigma_grid.shape)

    p_a0_grid = np.array([p_uniform(amin, amax)]*len(a0_grid))
    p_a0_grid = np.where((a0_grid>amin)&(a0_grid<amax),p_uniform(amin, amax),0)
    p_a0_grid = p_a0_grid[np.newaxis,:,np.newaxis]

    p_mbh_grid = np.array([p_uniform(mbhmin, mbhmax)]*len(logmbh_grid))
    p_mbh_grid = p_mbh_grid[np.newaxis,np.newaxis,:]

    integrand_mat = pa_sigma_grid*p_a0_grid*p_mbh_grid
    integrand_mat = np.where(A0 >= AF, integrand_mat, 0)
    paf0 = simpson(integrand_mat,a0_grid,axis=1)
    paf = simpson(paf0,logmbh_grid,axis=1)
    return af_grid, paf
...
def paf_analytic_allrates_mbhrange(p_a0,a0min,a0max,mbhmin,mbhmax,t,af_grid=None,
                        Naf=200,Na0=2000,uniform=False,rates=np.logspace(-1,1,50)):
    """given a sample of a0 and precalculated p_a0 function, 
        return a dictionary of predicted p_af for different rates"""
    if af_grid is None:
        af_grid = np.linspace(0, a0max + 3, Naf)
    
    # grid for a0
    a0_grid = np.linspace(0, np.max(af_grid)+3, Na0)
    logmbh_grid = np.linspace(mbhmin,mbhmax,20)
    # meshgrid for a0, af
    AF, A0, LMBH = np.meshgrid(af_grid, a0_grid, logmbh_grid, indexing='ij')

    # grid for p_a0
    if uniform:
        p_a0_vals = np.array([1/(a0max-a0min)]*len(a0_grid))
        p_a0_vals = np.where((a0_grid<a0max) & (a0_grid>a0min), 1/(a0max-a0min), 0)
    else:
        p_a0_vals = p_a0(a0_grid)
    # reshape to mesh grid
    p_a0_grid = p_a0_vals[np.newaxis,:,np.newaxis]
    p_mbh_grid = np.array([p_uniform(mbhmin, mbhmax)]*len(logmbh_grid))
    p_mbh_grid = p_mbh_grid[np.newaxis,np.newaxis,:]
    ...
    def optimize_inner(R):
        """function to parallelize p_af caculation for different rates"""
        # grid of integrand
        integrand_matrix = p_af_sigma_lmbh(AF, LMBH, t, A0, R) * p_a0_grid * p_mbh_grid
        # cut to integration limit: a0 >= af    
        integrand_matrix = np.where(A0 >= AF, integrand_matrix, 0)
        # integrate
        p_af0 = simpson(integrand_matrix, a0_grid, axis=1)
        p_af = simpson(p_af0, logmbh_grid, axis=1)
        return p_af
